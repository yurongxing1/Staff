		...
		<div class="form row">
			<form class="form-horizontal col-sm-offset-3 col-md-offset-3" id="login_form" action="javascript:void(0)">
				...
				<div class="col-sm-9 col-md-9">
					...
					<div class="form-group">
							<input class="form-control required" type="text" placeholder="年龄" value="${user.sex}" id="sex" name="sex" maxlength="8"/>
					</div>
					<div class="form-group">
							<select name ="depar_id" id="depar_id" class="form-control required">
									 <option value="0">---请选择部门---</option>
									 <c:forEach var="dep" items="${list}">
									 	<option value="${dep.id}" <c:if test="${dep.id==user.depar_id}">selected</c:if>>${dep.company}</option>
									 </c:forEach>
							</select>
					</div>
					<div class="form-group">
